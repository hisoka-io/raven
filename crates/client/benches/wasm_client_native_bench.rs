...
fn bench_one(
    ...
    let session =
        ClientSession::new(crs.clone(), sk.clone(), &mut session_sampler).expect("client session");
    ...
    for seed in 0..SEEDS {
        ...
        let es = Instant::now();
        let _plain = extract_response_rust(&crs, &state, &response, entry_bytes).expect("extract");
...
#[test]
#[ignore = "production-cell setup at d=2048 is ~12s; small plus production is ~30s. Trigger: \
            changing the client query-build or extract path, or the wasm-bindgen wrappers they \
            mirror."]
fn wasm_client_bench_native_per_cell() {
    ...
    // 512 B is NOT servable at ring_dim 256: InspiRING width is ceil(512/2) = 256, and the width
    // must be a power of two in [1, 128] at that ring. The row that used to sit here asked for
    // exactly that and would have panicked in `inspire_setup` the first time anything ran it —
    // which nothing ever did. The real 512-byte path record lives at ring_dim 2048 and is
    // measured below; 256 B is the widest legal record at d256.
    for (label, eb) in &[("bc-32B-d256", 32usize), ("wide-256B-d256", 256usize)] {
