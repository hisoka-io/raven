...
const LIST_KEY: [u8; 32] = [
    0xef, 0xc6, 0xdd, 0xb5, 0x9c, 0x09, 0x8a, 0x13, 0xfb, 0x2b, 0x61, 0x8f, 0xda, 0xe9,
    0x4c, 0x1c, 0x3a, 0x80, 0x7a, 0xbc, 0x8f, 0xb1, 0x83, 0x7c, 0x93, 0x62, 0x0c, 0x91,
    0x43, 0xee, 0x9e, 0x88,
];
...
async fn first_real_frozen_block_sync_retains_metadata_and_matches_every_root() {
    ...
    let rows = client.fetch_all_events(LIST_KEY).await.expect("frozen block fetch");
    ...
    for row in rows {
        ...
        assert_eq!(store.ppoi_imt_root(&LIST_KEY), Some(row.validated_merkleroot));
    }

    assert_eq!(store.ppoi_list_leaves_iter(&LIST_KEY).count(), expected_rows);
    let metadata = store
        .ppoi_event_metadata(&LIST_KEY, u32::try_from(FROZEN_BLOCK_ROWS - 1).expect("fits"))
