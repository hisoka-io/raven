...
const CELLS: &[(u32, usize, &str)] = &[
    (16, 32, "65k_x_32B"),   // T1 baseline / per-node alt
    (16, 64, "65k_x_64B"),   // γ=32
    (16, 128, "65k_x_128B"), // γ=64
    (16, 256, "65k_x_256B"), // γ=128
    (16, 512, "65k_x_512B"), // T2/T3 locked
    (17, 32, "131k_x_32B"),  // per-node candidate
];
...
fn per_shard_re_encode_cost_per_cell() {
    ...
    for (entries_log2, record_bytes, label) in CELLS {
        ...
        for sample_idx in 0..SAMPLES {
            // Mutate at a deterministic offset so each sample re-encodes a
            // slightly-different buffer (defeat constant-input caching).
        ...
        // Worst-case projection: full-DB re-encode = per_shard × total_shards.
