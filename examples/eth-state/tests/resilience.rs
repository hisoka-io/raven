...
/// A balance changed to exactly zero is tagged-present in the sidecar, so a pre-fold read is
/// served fresh by the SIDECAR and is byte-identical to the ledger, never falling back to the
/// stale main value. The fold keeps it correct.
#[test]
#[serial]
fn zero_balance_heals_via_tag() {
    ...
    // and the read is byte-identical to the ledger.
